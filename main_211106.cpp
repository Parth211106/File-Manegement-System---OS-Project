...
#include "FileSecurity_211110.cpp"
/*
            ...
            case 11:
                {
                    ...
                    FileSecurity fileSecurityobj(filePath);

                    // Open the file
                    if (!fileSecurityobj.Open()) {
                        return 1;
                    }

                    // Get the security information for the file
                    if (!fileSecurityobj.GetSecurityInfo()) {
                        return 1;
                    }

                    // Print the current access rights for the file
                    if (!fileSecurityobj.PrintCurrentAccessRights()) {
                        return 1;
                    }

                    // Set new access rights for the file
                    string sNewSecurityDescriptor = "D:(A;OICI;GRGW;;;WD)";
                    if (!fileSecurityobj.SetNewAccessRights(sNewSecurityDescriptor)) {
                        return 1;
                    }

                    // Cleanup and exit
                    fileSecurityobj.Cleanup();
